    ...
    this.openChat = function () {
        ...
        if (utils.getBroswer() != utils.broswer.IE6 && utils.getBroswer() != utils.broswer.IE7) {
            ...
            if(chatBox.getElementsByTagName("iframe")[0].contentWindow.postMessage){
                ...
                var chatWin = window.open(config.chatPage + "?v=" + config.version +"&clientId=" + config.clientId + "&fromUrl="+config.fromUrl+"&serviceStates="+utils.chatboxProp.isServiceTime+"&newWin=true&accessId=" + config.accessId+"&styleColor=" + styleColor+"&peers="+encodeURIComponent(JSON.stringify(peers))+"&peer="+ selectPeer+"&otherParams="+ config.otherParams, "chatWin", "width=380px,height=535px,toolbar=no, menubar=no, scrollbars=no, resizable=yes, location=no, status=no");
                chatWin.focus();
            }
            chatBtn.style.display = "none";
            utils.aniMove(chatBtn, {bottom: -chatBtn.offsetHeight, b: 0});
...
var utils = {
    ...
    setCookie: function (obj, exp, domain) {
